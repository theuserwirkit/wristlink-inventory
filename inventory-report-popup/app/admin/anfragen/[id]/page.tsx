...
import { QuoteReturnSection } from "@/components/admin/quote-return-section"
...
export default async function AnfrageDetailPage({
  ...
  const [events, rentalBooking] = await Promise.all([
    ["paid", "approved", "payment_pending", "submitted"].includes(quote.status)
      ? listFulfillmentEvents(quoteId).catch(() => [])
      : Promise.resolve([]),
    quote.booking_id && quote.config_json.modus === "miete"
      ? getBookingById(quote.booking_id)
  ...
  const bookingModalProps = needsReturnModal
    ? {
        groups: await getGroups(),
        batches: await getBatches(),
        customers: await getCustomers(),
        bases: await getBases(),
        inventoryLots: await getInventoryLots(),
        openRentals: await getOpenRentals(),
      }
    : null
    ...
    <div className="min-h-screen bg-background">
      <header className="border-b border-border bg-card">
        <div className="container mx-auto px-4 py-4 flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold">Anfrage #{quote.id}</h1>
          ...
          <div className="ml-auto flex flex-wrap gap-2 justify-end">
            <Button asChild variant="ghost" size="sm">
              <Link href="/admin/einstellungen/e-mails">E-Mail-Templates</Link>
            </Button>
          ...
          <CardContent>
            <dl className="grid gap-2 text-sm">
              <div className="flex justify-between">
                ...
                <dd>
                  {quote.booking_id ? (
                    <Link href={`/?booking=${quote.booking_id}`} className="text-primary underline">
                      #{quote.booking_id}
                    </Link>
                  ) : (
                    "–"
              ...
              {quote.return_booking_id && (
                <div className="flex justify-between">
                  <dt>Rückgabe-Buchung</dt>
                  <dd>
                    <Link href={`/protocol/${quote.return_booking_id}`} className="text-primary underline">
                      #{quote.return_booking_id}
                    </Link>
                  </dd>
                </div>
