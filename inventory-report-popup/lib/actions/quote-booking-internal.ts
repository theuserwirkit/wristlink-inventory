...
// Interne Buchungslogik für Angebote, ohne "use server"-Export: wird von serverseitigen,
// nicht Session-gebundenen Aufrufern verwendet (öffentlicher Angebots-Flow beim Anlegen einer
// Anfrage, Stripe-Webhook bei Zahlungseingang, Ablauf abgelaufener Reservierungen), siehe
// lib/quotes-internal.ts. Dadurch sind diese Funktionen nie Teil der Server-Actions-Oberfläche
// und können nicht direkt aus dem Client aufgerufen werden.
// Der einzige Client-erreichbare Einstiegspunkt ist ensureQuoteBooking in
// lib/actions/quote-booking.ts (auth-geschützt).

import { getDb } from "@/lib/db"
import { createN8nBooking } from "@/lib/actions/n8n-api"
import { createBookingInternal } from "@/lib/actions/bookings-internal"
import { getAvailabilityForGroupInternal } from "@/lib/actions/bookings"
...
export async function linkQuoteToBooking(quoteId: number, bookingId: number): Promise<void> {
  const sql = getDb()
  await sql`
...
export async function createQuoteHoldBooking(
  ...
  const bookingId = result.data?.id as number | undefined
  if (!bookingId) {
...
export async function finalizeQuoteBookingOnPayment(
  ...
  if (config.modus === "miete" && config.von) {
    if (quote.booking_id) {
      ...
      const newBookingId = result.data?.id as number | undefined
      if (!newBookingId) {
